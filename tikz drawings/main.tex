...
\begin{yquant}
    ...
    box {$IQC(\hat{x}_{ij})$} (q0, q1, q2, q3);
...
\node[] at ($(loc0)+(-2.6,-1)$) {$\{      \hat{x}_i, \hat{x}_j      \}$};


% draw arrow
\draw [thick, ->] ($(loc0)+(4.8,-1)$) -- ($(loc0)+(6.1,-1)$);
\node[] at ($(loc0)+(6.5,-1)$) {$\left\langle  \hat{y_i} \right\rangle$};
...
\node[node_small] at ($(NodeNet)+(-1,0.25)$) (dot) {$s_{ij}h_j^l$};
...
\node[] at ($(NodeNet.west)+(-0.5,-0.6)$)  {$s_{ij} $};
...
\begin{yquant}
    qubit { } a[8];
    box {$U(\hat\theta_0)$} (a[0], a[1]);
    discard a[0];
    box {$U(\hat\theta_1)$} (a[1], a[2]);
    discard a[1];
    box {$U(\hat\theta_2)$} (a[2], a[3]);
    discard a[2];
    box {$U(\hat\theta_3)$} (a[3], a[4]);
    discard a[3];
    box {$U(\hat\theta_4)$} (a[4], a[5]);
    discard a[4];
    box {$U(\hat\theta_5)$} (a[5], a[6]);
    discard a[5];
    box {$U(\hat\theta_6)$} (a[6], a[7]);
    discard a[6];
    measure a[7];
  \end{yquant}
\end{tikzpicture}
...
\begin{yquant}
    qubit { } a[2];
    [name=inits]
    box {$R_y(\theta_{i0})$} a[0];
    box {$R_y(\theta_{i1})$} a[1];
    [name=cnot]
    cnot a[1]  |  a[0];    
    discard a[0];
\end{yquant}

\node[] at ($(inits)+(-1.7,-0.31)$) {$U(\hat\theta_i)=$};
%\node[rectangle] at (cnot) (rect) { };
%\node[draw, dashed, fit=(inits)(cnot)] {};
\end{tikzpicture}
\end{center}
