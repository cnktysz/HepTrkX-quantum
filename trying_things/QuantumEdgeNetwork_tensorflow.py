...
def gradient(edge_array,label,theta_learn,class_weight,gradient_array,loss_array):
	grad = tf.zeros(tf.size(theta_learn), tf.float64)
	totol_loss = 0
	for i in range(len(label)):
...
def grad_fn(edge_array,y,theta_learn):
	...
	for thread in range(n_threads):
		...
		if thread==(n_threads-1):   
			p = multiprocessing.Process(target=gradient,args=(edge_array[start:,:],y[start:],theta_learn,class_weight,gradient_array,loss_array,))
		else:
			p = multiprocessing.Process(target=gradient,args=(edge_array[start:end,:],y[start:end],theta_learn,class_weight,gradient_array,loss_array,))   
		jobs.append(p)
		p.start()
	# WAIT for jobs to finish
	for proc in jobs: 
	...
	avg_loss = sum(loss_array) / n_edges
	
	with open(log_dir+'log_gradient.csv', 'a') as f:
		for item in avg_grad.numpy():
			f.write('%.4f,' % item)
		f.write('\n')
...
if __name__ == '__main__':
	...
	for epoch in range(n_epoch): 
		...
		for n_step in range(n_train):
			...
			grads, loss = grad_fn(B,y,theta_learn)
