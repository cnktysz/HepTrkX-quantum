...
dev1 = qml.device("qulacs.simulator", wires=6, gpu=USE_GPU)
...
def TTN_edge_forward(edge_array,theta_learn):
	...
	qml.RY(theta_learn[7],wires=2)
	qml.CNOT(wires=[1,2])
	# Third Layer
	qml.RY(theta_learn[8],wires=2)
...
# Use default.qubit for default pennylane simulation
# use tf.interface for TF integration
dev2 = qml.device("qulacs.simulator", wires=8, gpu=USE_GPU)
...
class EdgeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='EdgeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[11,],minval=0,maxval=4*np.pi,dtype=tf.float64))
...
class NodeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='NodeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[15,],minval=0,maxval=4*np.pi,dtype=tf.float64))
