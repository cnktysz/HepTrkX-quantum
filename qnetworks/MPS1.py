...
def node_forward(node_array,theta_learn):
	...
	for i in range(len(node_array[:,0])):
		out = tf.constant(np.pi*(1-MPS_node_forward(node_array[i,:],theta_learn))/2.,dtype=tf.float64)
		outputs.append(out)
...
class EdgeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='EdgeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[14,],minval=0,maxval=np.pi*4,dtype=tf.float64))
...
class NodeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='NodeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[22,],minval=0,maxval=np.pi*4,dtype=tf.float64))
