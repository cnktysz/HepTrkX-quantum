...
def MPS_edge_forward(edge_array,theta_learn):
	...
	qml.CNOT(wires=[0,1])
	qml.RY(theta_learn[2],wires=1)
	qml.RY(theta_learn[3],wires=2)
	qml.CNOT(wires=[1,2])
	qml.RY(theta_learn[4],wires=2)
	qml.RY(theta_learn[5],wires=3)
	qml.CNOT(wires=[2,3])
	qml.RY(theta_learn[6],wires=3)
	qml.RY(theta_learn[7],wires=4)
	qml.CNOT(wires=[3,4])
	...
	qml.CNOT(wires=[5,6])
	qml.RY(theta_learn[12],wires=6)
	qml.RY(theta_learn[13],wires=7)
	qml.CNOT(wires=[6,7])
...
def MPS_node_forward(node_array,theta_learn):
	# Takes the input and learning variables and applies the
	# network to obtain the output
	# STATE PREPARATION
	for i in range(12):
	...
	qml.RY(theta_learn[13],wires=7)
	qml.CNOT(wires=[6,7])
	qml.RY(theta_learn[14],wires=7)
	qml.RY(theta_learn[15],wires=8)
	qml.CNOT(wires=[7,8])
	qml.RY(theta_learn[16],wires=8)
	qml.RY(theta_learn[17],wires=9)
	qml.CNOT(wires=[8,9])
	qml.RY(theta_learn[18],wires=9)
	qml.RY(theta_learn[19],wires=10)
	qml.CNOT(wires=[9,10])
	qml.RY(theta_learn[20],wires=10)
	qml.RY(theta_learn[21],wires=11)
	qml.CNOT(wires=[10,11])
	# return expectation value of the circuit
	return qml.expval(qml.PauliZ(wires=11))
##################################################################################################
def edge_forward(edge_array,theta_learn):
	...
	for i in range(len(edge_array[:,0])):
		out = tf.constant((1-MPS_edge_forward(edge_array[i,:],theta_learn))/2.,dtype=tf.float64)
		outputs.append(out)
...
def node_forward(node_array,theta_learn):
	...
	for i in range(len(node_array[:,0])):
		out = tf.constant(2*np.pi*(1-MPS_node_forward(node_array[i,:],theta_learn))/2.,dtype=tf.float64)
		outputs.append(out)
...
class EdgeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='EdgeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[14,],minval=0,maxval=np.pi*2,dtype=tf.float64))
...
class NodeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='NodeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[22,],minval=0,maxval=np.pi*2,dtype=tf.float64))
