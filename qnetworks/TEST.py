...
def TTN_edge_forward(theta_learn):
	# First Layer
...
def TTN_node_forward(theta_learn):
	# First Layer
	qml.RY(theta_learn[0],wires=0)
	qml.RY(theta_learn[1],wires=1)
	qml.CNOT(wires=[0,1])
...
##################################################################################################
def edge_forward(theta_learn):
	outputs = []
	for i in range(len(theta_learn[:,0])):
		out = tf.constant((1-TTN_edge_forward(theta_learn[i,:]))/2.,dtype=tf.float64)
		outputs.append(out)
	return tf.stack(outputs) # output is between [-1,1]
##################################################################################################
def node_forward(theta_learn):
	outputs = []
	for i in range(len(theta_learn[:,0])):
		out = tf.constant((1-TTN_node_forward(theta_learn[i,:]))/2.,dtype=tf.float64)
		outputs.append(out)
	return tf.stack(outputs) # output is between [-1,1]
##################################################################################################
class EdgeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='EdgeNet'):
		...
		self.layer = tf.keras.layers.Dense(15,input_shape=(8,),activation='sigmoid')
	def call(self,X, Ri, Ro):
		...
		return edge_forward(self.layer(B)*2*np.pi)
##################################################################################################
class NodeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='NodeNet'):
		...
		self.layer = tf.keras.layers.Dense(23,input_shape=(12,),activation='sigmoid')
	def call(self, X, e, Ri, Ro):
		...
		return node_forward(self.layer(M)*2*np.pi)
##################################################################################################
