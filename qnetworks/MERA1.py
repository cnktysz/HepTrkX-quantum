...
class EdgeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='EdgeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[21,],minval=0,maxval=np.pi*2,dtype=tf.float64))
...
class NodeNet(tf.keras.layers.Layer):
	def __init__(self, config, name='NodeNet'):
		...
		self.theta_learn =  tf.Variable(tf.random.uniform(shape=[33,],minval=0,maxval=np.pi*2,dtype=tf.float64))
